...
## [v0.32] – Build 32
- **Fixed** port conflict check order: the conflict indicator (`⚡ Port in use by: ...`) now takes priority over the raw `listen tcp...` error that the SSH manager records when binding fails

## [v0.31] – Build 31
- **Added** port conflict detection in status view: when a local tunnel port is already occupied by another active connection, the status shows `⚡ Port in use by: [Name]` instead of marking the tunnel as active

## [v0.30] – Build 30
- **Added** `r` key in status view fully removes a deployed SSH key: removes the public key from the server's `authorized_keys`, deletes local private + public key files, resets the connection to password auth, disconnects, and returns to the connection list
- **Changed** help text updated from `r:Schlüssel` to `r:Schlüssel entfernen` / `r:Remove key`
